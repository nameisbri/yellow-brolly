 ...
 */
export default function HeroG_Alt() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      tl.fromTo('.hga-frame-1', { x: 300, rotation: 8, opacity: 0 }, { x: 0, rotation: 3, opacity: 1, duration: 1 })
        .fromTo('.hga-frame-2', { x: 350, rotation: -5, opacity: 0 }, { x: 0, rotation: -2, opacity: 1, duration: 1 }, 0.15)
        .fromTo('.hga-frame-3', { x: 280, rotation: 6, opacity: 0 }, { x: 0, rotation: 1, opacity: 1, duration: 1 }, 0.3);

      // Mobile illustration
      tl.fromTo('.hga-mobile-illust', { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.8 }, 0.5);

      // Headline lines stagger in
      tl.fromTo('.hga-line', { x: -60, opacity: 0 }, { x: 0, opacity: 1, duration: 0.8, stagger: 0.1 }, 0.3);

      // Bottom content
      tl.fromTo('.hga-sub', { opacity: 0 }, { opacity: 1, duration: 0.5 }, 0.9)
        .fromTo('.hga-cta', { opacity: 0, y: 15 }, { opacity: 1, y: 0, duration: 0.5 }, 1.1);
    }, sectionRef);
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      gsap.to('.hga-stack', { x: x * 25, y: y * 15, duration: 1.2, ease: 'power2.out' });
      gsap.to('.hga-text', { x: x * -8, y: y * -5, duration: 1.2, ease: 'power2.out' });
    };
    ...
    <section ref={sectionRef} className="relative overflow-hidden bg-yellow-primary min-h-screen flex items-center cursor-default">
      <div className="container mx-auto px-6 md:px-8 lg:px-12 max-w-7xl relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-center">
          ...
          <div className="lg:col-span-8 hga-text will-change-transform">
            <span className="hga-line inline-block text-black/50 text-sm font-semibold tracking-[0.3em] uppercase mb-8">
              Yellow Brolly Co
            </span>
            ...
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-[4.25rem] xl:text-[5.25rem] font-display font-bold leading-[1.05] tracking-tight text-black mb-8">
              {'When growth gets complex, we help you move forward.'.split(' ').map((word, i) => (
                <span key={i} className="hga-line inline-block mr-[0.22em] last:mr-0">{word}</span>
              ))}
            </h1>

            {/* Mobile illustration — single representative, visible below lg */}
            <div className="hga-mobile-illust flex gap-4 items-center mb-8 lg:hidden">
              <img src="/images/brand/winged-pencil.png" alt="" aria-hidden="true"
                className="w-20 h-20 object-contain opacity-[0.25]" />
              <img src="/images/brand/magnifying-glass.png" alt="" aria-hidden="true"
            ...
            <p className="hga-sub text-lg md:text-xl text-black/60 max-w-lg mb-10 leading-relaxed">
              We help organizations strengthen leadership, modernize operations, and implement change that lasts.
            </p>
            <Button to="/contact" variant="dark" size="lg" className="hga-cta">
          ...
          <div className="lg:col-span-4 hga-stack relative hidden lg:block will-change-transform" style={{ height: '580px' }}>
            {/* Pencil — top, pushed left so it overlaps with coffee person */}
            <div className="hga-frame-1 absolute -top-4 -right-4 will-change-transform"
            ...
            <div className="hga-frame-2 absolute top-[28%] -left-8 will-change-transform"
            ...
            <div className="hga-frame-3 absolute -bottom-[15%] -right-[5%] will-change-transform"
